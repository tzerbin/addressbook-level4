...
[IMPORTANT]
====
You can only delete an appointment when you are listing appointments. Refer to
<<Listing appointments: `listAppointment` [Since v1.3]>> for more information.
====
...
Alias: `la` +
Format: `listAppointment [STARTDATE ENDDATE]`
...
* If an appointment starts within the date range specified but ends after the `ENDDATE`, it will also be shown.
* If an appointment ends within the date range specified but starts before the `STARTDATE`, it will also be shown.
****
...
into our `v2.0` product and hence you should not click on the dates. If you want to view a certain date's appointment,
