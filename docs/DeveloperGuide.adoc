...
|`* * *` |user |have multiple calendar views by day, week, and month |check appointments in different time frames
...
|`* * *` |user |save appointment |retrieve information of the appointments made
...
|`* * *` |user |see the location of a given appointment in a map |plan for travel

|`* * *` |user |see various landmarks around a specific location in a road map |understand better the roads around the location

|`* * *` |user |see various landmarks around a specific location in a satelite map |see in real time the actual layout of the surrounding

|`* * *` |user |zoom in and out of the map of given location in a map |view the location in different level of details

|`* * *` |user |see the best route of travel by driving between two locations in a map |plan for the shortest travel

|`* * *` |user |know if two locations can be reached by driving |foresee any problems and plan ahead

|`* * *` |user |know the estimated distance between two locations by driving |plan for my schedules more effectively

|`* * *` |user |know the estimated time of travel between two locations by driving |plan for my schedules more effectively
...
1.  User keys in a partially complete command which only has one match in the CelebManager
2.  CelebManager shows the matched complete command
...
2.  CelebManager opens the platform of the emailing website
...
2.  CelebManager lists out contacts with the specified tag
...
2.  CelebManager requests the user to confirm black list
3.  User confirms
4.  CelebManager adds the phone number to black list
...
2.  CelebManager updates the size of the UI indicated
...
2.  CelebManager adds the appointment to the calendar
...
2.  CelebManager shows the appointment details
...
2.  CelebManager sends a pre-drafted email containing appointment details to the attendees
...
2.  CelebManager removes the appointment
...
2.  CelebManager changes appointment details and displays new appointment details to user
+
Use case ends.
...
2.  CelebManager sends reminder whenever the user opens it after the specified time till appointment timing
...
2.  CelebManager cancels the alarm
...
2.  CelebManager outputs payments made and pending
...
+
[none]
** 2a1. CelebManager shows a message that says no payment.
+
Use case ends.
...
2.  CelebManager provide the list of appointments User has.
3.  User requests to associate relevant parties to one of the appointments from the list.
4.  CelebManager requests input of the relevant parties’ names.
5.  User enters the names of the parties.
6.  CelebManager adds the names to the appointments.
7.  CelebManager provides confirmation.
+
Use case ends.
...
+
[none]
** 4a1. CelebManager requests User to create a new contact.
+
[none]
...
+
[none]
** a1. CelebManager provides status.
+
Use case ends.
...
4.  CelebManager requests input of the points of contact’ names.
5.  User enters the names of the parties.
6.  CelebManager adds the names to the appointments.
7.  CelebManagerr provides confirmation.
...
4.  CelebManager requests input of the payment amount.
5.  User enters the payment amount.
6.  CelebManager adds payment amount to the appointments.
7.  CelebManager provides confirmation.
...
+
[none]
** 4a1. CelebManager requests User to provide valid input.
+
[none]
