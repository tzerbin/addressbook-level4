...
This section includes instructions to test the `addAppointment` command of the application.
[NOTE]
...
.. Type `aa n/Oscar` and press enter. Then type `la` and press enter. +
...
.. Type `aa n/Grammy sd/10-04-2018 st/15:00` and press enter. Then type `la` and press enter. +
...
.. Type `aa n/Dentist appointment sd/11-04-2018 st/16:00 ed/12-04-2018 et/09:00 c/n p/k` (where n corresponds to the index of a celebrity
...
.. Type `aa n/Lunch meeting c/x` (where x refers to an invalid celebrity index) and press enter. +
    _Expected: No appointment is added. Error details shown in the status message._
.. Type `aa n/Dinner l/???` and press enter. +
    _Expected: No appointment is added. Error details shown in the status message._
...
=== Estimated route between two location on map
...
=== Removing a tag
This section includes instructions to test the `removeTag` command of the application.
[NOTE]
...
.. Type `removeTag celebrity` in command box and press enter +
   _Expected: `celeberity` tag is not removed. Error details shown in the status message._
.. Type other incorrect `removeTag` commands such as `removeTag`, `removeTag x` (where `x` is a tag that nobody has in the full contact list or an invalid tag name) in command box and press enter +
   _Expected: No tag is removed. Error details shown in the status message._
...
.. Type correct deletAppointment commands such as `deleteAppointment 1` in command box and press enter +
