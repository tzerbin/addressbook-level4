= Celeb Scheduler - Developer Guide
:toc:
:toc-title:
...
The main diffrence between both model is shown below in the two diagrams.
...
The snippet below shows the state of `tags`:

.Image of `tags` with `friends` tag and `husband` tag
image::BeforeRemoveTag.png[width="400"]

After the successful execution of `removeTag friends`, `tags` will be updated to the diagram below:

.Image of `tags` after the execution of `removeTag friends`
image::AfterRemoveTag.png[width="400"]
...
** Cons: Requires he creation of additional exception class and exception handling methods.
...
Dev Ops advocates automation and monitoring at all steps of software construction. This section illustrates how we adopt this practice in developing AddressBook.
...
1.  User keys in a partially complete command which only has one match in the AddressBook
2.  AddressBook shows the matched complete command
3.  User presses tab to let AddressBook auto-fill the rest of the command
...
2.  AddressBook undoes the latest executed command that mutates the data
...
2.  AddressBook redoes the latest executed undo command
...
2.  AddressBook opens the platform of the emailing website
...
2.  AddressBook lists out contacts with the specified tag
...
2.  AddressBook requests the user to confirm black list
3.  User confirms
4.  AddressBook adds the phone number to black list
...
2.  AddressBook updates the size of the UI indicated
...
2.  AddressBook adds the appointment to the calendar
...
2.  AddressBook shows the appointment details
...
2.  AddressBook sends a pre-drafted email containing appointment details to the attendees
...
2.  AddressBook removes the appointment
...
2.  AddressBook changes appointment details and displays new appointment details to user
+
Use case ends.
...
2.  AddressBook sends reminder whenever the user opens it after the specified time till appointment timing
...
2.  AddressBook cancels the alarm
...
2.  AddressBook outputs payments made and pending
...
+
[none]
** 2a1. AddressBook shows a message that says no payment.
+
Use case ends.
...
2.  AddressBook provide the list of appointments User has.
3.  User requests to associate relevant parties to one of the appointments from the list.
4.  AddressBook requests input of the relevant parties’ names.
5.  User enters the names of the parties.
6.  AddressBook adds the names to the appointments.
7.  AddressBook provides confirmation.
+
Use case ends.
...
+
[none]
** 4a1. AddressBook requests User to create a new contact.
+
[none]
...
+
[none]
** a1. AddressBook provides status.
+
Use case ends.
...
4.  AddressBook requests input of the points of contact’ names.
5.  User enters the names of the parties.
6.  AddressBook adds the names to the appointments.
7.  AddressBook provides confirmation.
...
4.  AddressBook requests input of the payment amount.
5.  User enters the payment amount.
6.  AddressBook adds payment amount to the appointments.
7.  AddressBook provides confirmation.
...
+
[none]
** 4a1. AddressBook requests User to provide valid input.
+
[none]
...
.  Should be backward compatible with data produced by earlier versions of the AddressBook
