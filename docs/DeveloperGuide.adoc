...
address book, and prints out how many persons are affected by this removal.

The `RemoveTagCommand` class inherits from the `UndoableCommand` class and hence supports the `undo` and `redo` features.
...
----
From the snippet above, it can be seen that `RemoveTagCommand` disallows removal of `celebrity` tag, or a non-existing
...
* **Alternative 1 (current choice):** Output the `celebrity` tag cannot be removed
...
* **Alternative 2:** Removes `celebrity` tag and clears all calendars
...
After input of the command, we will have as shown below (Figure 21):
...
and `deleteAppointment` in `Model`. It supports the deletion of an appointment from an existing calendar.
...
* **Alternative 1 (current choice):** Switches back to combined calendar view
...
* **Alternative 2:** Stays at the appointment list view and shows an empty list
...
appointment to list.
* **Alternative 3:** Switches back to combined calendar view and goes to the day when the deleted appointment should
happen
...
in the `ResultDisplayPanel` UI by displaying the `Appointment` details. The specific `Appointment` is selected using an index based on the list generated by `ListAppointmentCommand`.
...
in the `MapPanel` UI by updating the state of the `MapPanel`. This is done by re-centering the `MapPanel` to the new route.
...
An example would be the details of unit number (e.g #11-111) which will not be accurate for this command.
...
Any subsequent `estimateRoute` command will remove any existing marker or route and update the `MapPanel` with the new route.
