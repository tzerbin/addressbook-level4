...
As this command relies on the list generated by `ListAppointmentCommand`, it retrieves the start (earliest) and end (latest) date
from it which is used to generate the appointment list internally from the `StorageCalendar` in `Model`.
