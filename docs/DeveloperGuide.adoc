...
in the `ResultDisplayPanel` UI by displaying the `Appointment` details. The specific `Appointment` is selected using an index based on the list generated by `listAppointment` function.
